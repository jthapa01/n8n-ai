...
test.describe("Credential Creation", () => {
  ...
  test("Create OpenAI credential", async ({ page }) => {
    const testName = `OpenAI_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    try {
      // Fill form (OpenAI should be pre-selected)
      await page.fill('input[name="name"]', testName);
      await page.fill('input[name="value"]', "sk-test123456789");
