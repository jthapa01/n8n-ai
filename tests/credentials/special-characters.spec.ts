...
test.describe("Error Handling and Edge Cases", () => {
  ...
  test("Special characters in credential data", async ({ page }) => {
    ...
    try {
      ...
      const specialName = `Special_Key_&_Co_#1@test!_${Date.now()}`;
      await page.fill('input[name="name"]', specialName);
      await page.fill('input[name="value"]', "sk-test_key-123@domain.com");

      // Try to submit
      await page.click('button[type="submit"]', { timeout: 10000 });

      console.log("✅ Special characters test interaction completed");
    } catch (error) {
      console.log(
        `Special characters test completed with issues: ${error.message}`,
      );
    }
  });
});
